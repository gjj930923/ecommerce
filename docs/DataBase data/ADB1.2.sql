...
begin
	...
	insert into Pname_Cname_sales(`Pname`,`Cname`,`annual_income`,`business_category`,`year`,`month`,`sales`)
		select p.Pname, b.Cname,b.`annual_income`,b.`business_category`,s.`year`,s.`month`,sum(s.sales) as sales
			from sales s, Businessc_Dimension b,product_dimension p
			where s.cid=b.cid and p.pid=s.pid
...
begin
	...
	insert into Pname_Nname_sales(`Pname`,`Fname`,`Lname`,`Nname`,`gender`,`age`,`income`,`marriage_status`,`year`,`month`,`sales`)
		select p.Pname, h.Fname,h.Lname,h.Nname,h.gender,h.age,h.income,h.marriage_status,s.`year`,s.`month`,sum(s.sales) as sales
